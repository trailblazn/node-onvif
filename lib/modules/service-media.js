...
/* ------------------------------------------------------------------
* Method: setVideoSourceConfiguration(params[, callback])
* - params:
*   - ConfigurationToken | String | required |
*
* {
...
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.setVideoSourceConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		...
		let soap_body = '';
		soap_body += '<trt:SetVideoSourceConfiguration>';
		soap_body += '	<trt:Configuration token="' + params['ConfigurationToken'] + '">';
		soap_body += '	    <tt:Name>' + params['Name'] + '</tt:Name>';
		soap_body += '	    <tt:UseCount>' + params['UseCount'] + '</tt:UseCount>';
		soap_body += '	    <tt:SourceToken>' + params['SourceToken'] + '</tt:SourceToken>';
		soap_body += '	    <tt:Bounds height="' + params['Bounds']['height'] + '" width="' + params['Bounds']['width'] + '" y="' + params['Bounds']['y'] + '" x="' + params['Bounds']['x'] + '"></tt:Bounds>';
		// optional extensions - when supported by device
		if (params['Extension']) {
			soap_body += '		<tt:Extension>';
			// rotate video stream - when supported by device
			if (params['Extension']['Rotate']) {
				soap_body += '			<tt:Rotate>';
				soap_body += '				<tt:Mode>ON</tt:Mode>';
				soap_body += '				<tt:Degree>' + params['Extension']['Rotate']['Degree'] + '</tt:Degree>';
				soap_body += '			</tt:Rotate>';
			}
			soap_body += '		</tt:Extension>';
		}
		...
		mOnvifSoap.requestCommand(this.oxaddr, 'SetVideoSourceConfiguration', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
